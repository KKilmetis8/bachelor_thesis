...
Central_Density = np.logspace(4,7,10) # Range of different central densities, 10^4-10^7 
...
for i in range(len(Central_Density)):
    ...
    plt.yscale('log')
    plt.savefig('wdn_pr.pdf', format='pdf')
    ...
    plt.savefig('wdn_mr.pdf', format='pdf')
#%%    
plt.figure(3) # Total Mass vs WD radius
plt.plot(R_tot, M_tot, 'o')
plt.title('Total Masses of Newtonian WD\'s of varying central densities')
plt.xlabel("R $[Km]$")
plt.ylabel("Mass $[M_{\oplus}]$")
plt.savefig('wdn_maxmass.pdf', format='pdf')
