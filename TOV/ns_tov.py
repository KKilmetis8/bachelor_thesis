...
Central_Density = np.logspace(14,17,30) # Range of different central densities. 
...
for i in range(len(Central_Density)):
    ...
    plt.yscale('log')
    plt.savefig('tov_pr.pdf', format='pdf')
    ...
    plt.savefig('tov_mr.pdf', format='pdf')
 #%%
plt.figure(3) 
plt.plot(R_tot, M_tot, 'o')
plt.title('Total Masses of NS\'s of varying central densities, using a Polytrope EoS')
plt.xlabel("R $[Km]$")
plt.ylabel("Mass $[M_{\odot}]$")
plt.savefig('tov_maxmass.pdf', format='pdf')