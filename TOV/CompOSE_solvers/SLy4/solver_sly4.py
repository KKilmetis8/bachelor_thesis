...
df = pd.read_csv('eos_sly4.thermo',sep='\s+', header=None) # Reads the Compose EoS, P/nb is in MeV
names = [ 'it','inb','iyq','P/nb' , 
         's/nb','mub/mn-1','muq/mn','ml/mn','f/nb*mn-1','e/nbmn-1','extra',]
df.columns = names # Change the column names

df_nb = pd.read_csv('eos_sly4.nb') # The nb is on another file, units [fm^-3]
...
Central_Density = 2.8e15 # <4e15 to be inside of interpolation range
M_tot = []
R_tot = []
Central_Pressure = pressure_inter(Central_Density)    
S_0 = [Central_Pressure,0.001] # Initial values [Pressure, Mass]
...
Central_Density = np.logspace(np.log10(5e14),np.log10(2.8e15),30) # <5e15 to be inside of interpolation range. The Liquid core starts at 1.4e14
M_tot = []
R_tot = []
for i in range(len(Central_Density)):
    ...
    fig = plt.plot(x,p_sol, label='Central Density: ' + den_text + '$\hspace{1} [g/cm^3]$')
    plt.legend(loc='best')
    plt.title('Pressure of a Neutron Star, SLy4 CompOSE')
    plt.xlabel("R $[Km]$")
    plt.ylabel("Pressure $[erg/cm^3] $")
    plt.yscale('log')
    plt.ylim(1e26)
    ...
    fig = plt.plot(x,m_sol, label='Central Density: ' + den_text + '$\hspace{1} [g/cm^3]$')
    plt.legend(loc='best')
    plt.title('Mass of a Neutron Star, SLy4 CompOSE')
    plt.xlabel("R $[Km]$")
...
plt.title('Total Masses of NS\'s of varying central densities, SLY4')
plt.xlabel("R $[Km]$")
plt.ylabel("Mass $[M_{\odot}]$")
...
R_sly4 = R_tot
den_sly4 = den_cgs
pres_sly4 = pressure_inter(den_cgs)
text_sly4 = "{:.1e}".format(Central_Density[1]) + ' ' + '-' +' ' + "{:.1e}".format(Central_Density[-1])