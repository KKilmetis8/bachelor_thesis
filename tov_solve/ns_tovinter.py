...
pressure_inter =  interp1d(df.loc[:,'rhog/cm3'],df.loc[:,'Perg/cm3'], kind='cubic')
den_inter = interp1d(df.loc[:,'Perg/cm3'],df.loc[:,'rhog/cm3'], kind='cubic')
#%% One Neutron Star
...
def starend(t,y):
    return y[0] - 4e24 # Stop integrating when P = 4e24, otherwise we get out of Interpolation range. (3e34 compiles)
starend.terminal = True # Stop the integration when starend event is True
starend.direction= -1 # Looks for sign changes + -> -

Central_Density = 4e15 # <5e15 to be inside of interpolation range
M_tot = []
R_tot = []
...
Central_Density = np.logspace(np.log10(2e14),np.log10(4e15),20) # <5e15 to be inside of interpolation range. The Liquid core starts at 1.4e14
M_tot = []
R_tot = []
